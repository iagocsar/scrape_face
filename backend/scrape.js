...
async function scrape(config, log) {
      ...
      log({
            ...
            message: `${proxies ? proxy : 'No proxy'}`,
...
async function getListings(
      ...
      try {
            ...
            //
            // const page = await fbLogin(browser, login, pass);
            ...
            ); // Set user-agent to insure we don't receive mobile version.
            ...
            const listings = await page
                  ...
                  .then(async () => {
                        const products = await page.evaluate(() => {
                              let pageItems = Array.from(
                                    document.querySelectorAll(
                                          'div[style="max-width:1872px"] > div > div'
                                    )
                              );
                              pageItems.length === 0
                              ...
                              //pageItems.length = 7;
                              console.log(
                                    'Listings found: ' + pageItems.length
                              );
                              return pageItems.map((listing) => {
                                    ...
                                    const price = listing.querySelector('[dir]')
                                          ? listing.querySelector('[dir]')
                                                  .textContent
                                          : null;
                                    const location = listing.querySelector(
            ...
            log({
                  id: keyword.id,
                  keyword: keyword.keyword,
                  message: 'Listings found: ' + listings.length,
                  time: Math.floor(Date.now() / 1000),
            });
            return listings;
      } catch (error) {
